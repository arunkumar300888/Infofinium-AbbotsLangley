...
<div id="docReview" class="my-document complete box-content">
			...
			<div align="right">
				<input type="button" value="Back" id="btnBackToList" onclick="loadMetadata('${formName}', '${path}', '${caseId}', '${stepList}','false');" class="download" />
			</div>
...
<div id="docProperties" class="my-document complete box-content">
			...
			<div align="right">
				<input type="button" value="Back" id="btnBackToList" onclick="loadDocuments();"  class="download" /></div>
		...
		<small id="docNameheader" class="title1">${formName}  > ${ActionStatus}</small></div>
			...
			style="width: 980px">
					  ...
					  <c:if test="${(stepAssigned == true)}">
						...
						<c:if test="${canShowAdminUpdateProperties}">
                         ...
                         <input type="button" class="butn" value="Update Properties" id="updateProperties"
								...
								onClick="updateLinks('${documentId}','${stepList}','${path}','${docName}','${caseId}','false');" style="width: 142px;" />
							<br /><br />
					</div>

					<!-- For Reviews -->
					<div id="review" align="left" style="height:auto">
						

						<%-- <input type="button" id="btnPrint" class="butn" value="Print" onClick="printClicked('${caseId}');" style="width: 50px;"> --%>
						<table id="reviewComments" class="display" style="height:auto;">
							<thead>
							<tr>
					...
					onClick="printClicked('${documentId}');"></div></th>
						...
						<form id="frmReviews" style="width:auto; align_content:center;">
								...
								<tr>
									<td valign="bottom" align="right">
										<input class="butnblue" type="button"
											   value="Save Comment" id="saveComment" onclick="postReviewClicked('false');"
											   style="width: 120px;" />
									</td>
								</tr>
							</table>
					...
					<c:if test="${(formAttached == true)}">
						<c:if test="${(stepAssigned == true)}">
							<input type="button" class="butn" value="Form"
								   id="btnForm" onclick="formBtnClicked('${userFormId}','false');"
								   style="width: 142px;" />
							<br />
						</c:if>
						...
						<input type="button" class="butn" value="Properties" id="btnProperties"
							   onclick="loadFormProperties('${formName}', '${path}', '${caseId}', '${stepList}','false');" style="width: 142px;" />
						<br />
						<%-- <c:if test="${canShowAdminAll}">  
						...
						<%-- <c:if test="${canShowAdminAll}">   --%>
							<input type="button" class="butn" value="History" id="btnhistory"
							   onclick="historyBtnFormClicked('${userFormId}');" style="width: 142px;" />
						<br />
						<%-- </c:if> --%>
						...
						<c:if test="${canShowClone}">
						<input type="button" class="butn" value="Clone" id="btnClone"
							onclick="cloneBtnClicked('${userFormId}');"	style="width: 142px;" />
							<br />
						</c:if>
						
						<%-- <c:if test="${canShowClone}">
						<input type="button" class="download" value="Revision"
							onclick="popupFormRevisionSelected('${userFormId}')"
							style="width: 142px;" />
						<br /><br /></c:if> --%>

                        <c:if test="${canShowAdminAll}">  
						<input type="button" class="butn" value="Comments  (${commentsCount})" id="btnComment"
							   onclick="reviewBtnClicked('${path}', '${formName}', '${caseId}', '${stepAssigned}', '${stepList}','false');"
							   style="width: 142px;" />
						<br />
						</c:if>
						...
						<c:if test="${(canShowAbandon == true)}">
							<input type="button" class="butn" value="Abandon" id="abandon"
								   onClick="abandonClicked('${stepId}','false');" style="width: 142px;" />
							<br />
						</c:if>
						...
						<c:if test="${(canPick == true) && (stepAssigned == true)}">
						 <c:if test="${canShowAdminAll}">  
							<input type="button" class="butn" value="Claim"
								   onClick="onPickFormClicked('${path}', '${formName}', '${caseId}', '${stepId}','false');" style="width: 142px;" />
								   </c:if>
