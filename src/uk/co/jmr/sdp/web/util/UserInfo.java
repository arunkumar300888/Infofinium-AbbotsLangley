...
public class UserInfo implements WfUserInfo {
	...
	private Set<String> groupRoles = new HashSet<String>();
	...
	public UserInfo(User user) {
		...
		for (Role r : rs) {
			if (r.getIsActive() == 'Y')
				this.roles.add(r.getRoleName());
		}
		Set<GroupRole> grs = user.getGroupRoles();
		for (GroupRole gr : grs) {
			if (gr.getGroup().getIsActive() == 'Y' && gr.getRole().getIsActive() == 'Y')
				this.groupRoles.add(gr.getRole().getRoleName() + "." + gr.getGroup().getGroupName());
		}
	}
